  ...
  final:
        ...
        case TOK.identifier:
            {
                ...
                parameterList.parameters.push(new AST.Parameter(loc, STC.parameter, t, token.ident, null, null, null));
    ...
    Lwhile:
        ...
        if (storageClass != 0 && token.value == TOK.identifier && n.value == TOK.assign)
        {
            ...
            nextToken();
            check(TOK.assign);
            param = new AST.Parameter(aloc, storageClass, at, ai, null, null, null);
        }
    ...
    AST.Expression parsePrimaryExp()
    {
        ...
        LabelX:
            ...
            if (token.value == TOK.leftParenthesis)
            {
                ...
                bool trailingComma;
                auto args = parseArguments(trailingComma);
                auto call = new AST.CallExp(loc, e, args);
                call.trailingComma = trailingComma;
                e = call;
                break;
            }
     ...
     * Assume current token is ',', '$(LPAREN)' or '['.
     */
    private void parseNamedArguments(AST.Expressions* arguments, AST.Identifiers* names, out bool trailingComma)
    {
        assert(arguments);
