...
extern (C++) final class TupleLiteralExp : Expression
{
    ...
    override bool equals(const RootObject o) const
    {
        ...
        if (!e)
            return false;
        if (auto te = e.isTupleLiteralExp())
        {
            if (elements.length != te.elements.length)
                return false;
            if (elements.length == 0 && !type.equals(te.type))
            {
                return false;
            }
    ...
    override Optional!bool toBool() // TODO: ok?
    {
        return typeof(return)();
    }
